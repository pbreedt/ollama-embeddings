...
func RunLLMWithEmbedding() {
	...
	collection, err := db.GetOrCreateCollection("Wikipedia", nil, chromem.NewEmbeddingFuncOllama(embeddingModel, "http://localhost:11434/api"))
	if err != nil {
		panic(err)
	}
	...
	if collection.Count() == 0 {
		...
		f, err := os.Open("./chromem/dbpedia_sample.jsonl")
		if err != nil {
			panic(err)
		}
		defer f.Close()
